    ...
    public class TtsRelayComponent : VHuman.IComponent
    {
        ...
        public void SetVisemeMapping(string visemeMapping)
        {
           ...
           // if (cerevoice != null) cerevoice.SetVisemeMapping(m_visemeMapping);
            //if (polly != null) polly.SetVisemeMapping(m_visemeMapping);
        }
...
/*            if (File.Exists("AWSSDK.Polly.dll"))
            {
                polly = new AWSPollyRelay();
                polly.Init(m_visemeMapping);
            }*/
...
/*            if (polly != null)
            {
                voices = polly.GetVoices();
                foreach (string voice in voices)
                {
                    Console.WriteLine("Adding '{0}' voice '{1}'", "polly", voice);
                    voicemap.Add(voice, polly);
                }
            }*/
        ...
        private void ProcessSpeechMessage(string message)
        {
            ...
            Console.WriteLine("voice message is !!!!!" + voice);
            ...
            #if (!__MonoCS__)
            if ( relay is MsSpeechRelay || relay is AWSPollyRelay)
            {
            ...
            Console.WriteLine("speechpart is!!!!!!!!!!" + speechPart.InnerXml+spID+"!!!!!!!!!!!!1");
            ...
            if ( relay is MsSpeechRelay || relay is AWSPollyRelay)
            {
                ...
                speechPart.SetAttribute("xml:lang", "zh-CN");
            }


            // strip characters that will throw exceptions when being written
            XmlNodeList speechNodes = speechPart.ChildNodes;
            foreach (XmlNode n in speechNodes)
            {
                if (n is XmlText)
                {
                    XmlText elem = (XmlText)n;
                    //if (elem.NodeType == XmlNodeType.Text)
                    {
                        string value = elem.Value;
                        Console.WriteLine("VALUE" + value  + "!!!!!!!!!!!!1");
                        elem.Value = HttpUtility.UrlDecode(value);
                       /* System.Text.Encoding utf8, gbk;
                        utf8 = System.Text.Encoding.GetEncoding("utf-8");
                        gbk = System.Text.Encoding.GetEncoding("gb2312");
                        byte[] utf;
                        utf = utf8.GetBytes(value);*/
                        //utf = System.Text.Encoding.Convert(utf8, gbk, utf);
                       /* elem.Value =gbk.GetString(utf);
                        Console.WriteLine("utfis" + gbk.GetString(utf) + "!!!!!!!!!!!!1");*/
                        elem.Value = elem.Value.Replace(":", "");
                        Console.WriteLine("elem.Value is!!!!!!!!!!" + elem.Value + "!!!!!!!!!!!!1");
                    }
                }
